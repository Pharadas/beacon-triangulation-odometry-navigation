...
def callback(msg):
    ...
    if should_start_writing_to_topic:
        ...
        phis_N = int((phis_length * N) / thetas_length)
        image = np.zeros((phis_N, N))

        for i in range(len(rs)):
            if (thetas[i] > min_thetas and thetas[i] < max_thetas) and (rs[i] < 10):
        ...
        matrix_image_msg.data = image
        matrix_image_msg.layout.data_offset = 0
        publisher.publish(list(matrix_image_msg))
